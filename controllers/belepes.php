...
class Belepes_Controller
{
            ...
            if ($form_type == 'registration') {
                ...
                if (empty($validation_errors)) {
                    $this->felhasznalo_model->felhasznalo_hozzaad([
                        ...
                        'jelszo'        => $_POST['jelszo'],
                        'szerepkor_id'  => $this->getRegisteredVisitorRoleId()
                    ]);
    ...
    private function getRegisteredVisitorRoleId()
    {
        $dbh = get_db_connection();
        $stmt = $dbh->prepare('SELECT id FROM szerepkorok WHERE kod = "regisztralt_latogato"');
        $stmt->execute([]);
